...
export default class QueryStatus extends PubSub {
  ...
  attached() {
    ...
    this.subscribe(this.notifications.i2b2.networkIdReceived, (d) => {
      ...
      this.publish(this.commands.shrine.fetchQuery, {
        networkId, timeoutSeconds: TIMEOUT_SECONDS, dataVersion: DEFAULT_VERSION
      });
    });
    ...
    this.subscribe(this.notifications.shrine.queryReceived, (data) => {
      const {
        query, nodes, dataVersion = DEFAULT_VERSION, query: { networkId = null } 
      } = data;
...
const initialState = n => ({
  status: {
    query: {
      networkId: null, queryName: null, updated: null, complete: false
    }
  },
  nodes: [],
  hubMsg: hubMsgTypes.WAITING_ON_RESPONSE
});
