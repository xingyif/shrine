...
object KeyStoreEntry {
  def extractCertHolder(
      signedBytes: Array[Byte]): Option[X509CertificateHolder] = {
    import scala.collection.JavaConversions._
    val signedData = new CMSSignedData(signedBytes)
    val store =
      signedData.getCertificates.asInstanceOf[Store[X509CertificateHolder]]
    val certCollection = store.getMatches(SelectAll)
    certCollection.headOption
  }

  def getCommonNameFromCert(
      certHolder: X509CertificateHolder): Option[String] = {
    for {
...
case class RemoteSite(url: String,
