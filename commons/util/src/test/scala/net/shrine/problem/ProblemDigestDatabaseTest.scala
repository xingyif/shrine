...
/**
  ...
  */
final class ProblemDigestDatabaseTest extends FlatSpec with BeforeAndAfter with ScalaFutures with Matchers {
  val ten = Span(10, Seconds)
  implicit val defaultPatience =
    PatienceConfig(timeout = ten, interval = Span(10, Millis))
  ...
  before {
    ...
    Await.ready(db.run(schema.create), ten)
  ...
  after {
    Await.ready(db.run(schema.drop), ten)
    // After dropping the tables, there should be nothing left, thus this throws a
    // java sql exception (since the tables don't exist)
    whenReady(db.run(problems.result).failed) { e =>
      e shouldBe a[java.sql.SQLException]
    }
    db.close()
  }
  ...
  "The Database" should "Connect without any problems" in {
    ...
    Await.ready(db.run(DBIO.seq(problems ++= problemDigests, suppliers ++= supplierTuples)), ten)
