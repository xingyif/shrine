...
final case class ShrineResource(shrineRequestHandler: ShrineRequestHandler) extends Loggable {
  ...
  def runQuery(
    ...
    val topicIdAndNameOption = (topicIdOption,topicNameOption) match {
    ...
    performAndSerialize(_.runQuery(RunQueryRequest(projectId, waitTime, authorization, -1, topicIdAndNameOption, outputTypes.toSet, queryDef), shouldBroadcast))
  }
